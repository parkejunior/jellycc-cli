...
describe('E2E: JellyCC Check Menu', () => {
  ...
  test('Should open stream selection menu, drop a track, and update action plan to Remux', async () => {
    ...
    const exitCode = await cli.waitForExit();
    expect(exitCode).toBe(0);
    expect(cli.getOutput()).toContain('The file only requires cleanup (Remux). You discarded 1 stream(s).');
  });
  ...
  test('Should open tags editor, change language, and update the suggested command', async () => {
    ...
    const exitCode = await cli.waitForExit();
    expect(exitCode).toBe(0);
    expect(cli.getOutput()).toContain('language="jpn"');
  });
  ...
  // Silence Scan
  test('Should bypass Silence Scan execution if media has no audio tracks', async () => {
    cli = new CliTester(['bun', 'run', 'src/index.ts', 'check', fixturePath('no_audio.mkv')], process.cwd());
    ...
    await cli.waitForText('Audio intact: No sudden drops detected.');
    await openExecutionMenu();
  ...
  test('Should execute Silence Scan successfully on a clean video and return to menu', async () => {
    cli = new CliTester(['bun', 'run', 'src/index.ts', 'check', fixturePath('silence_clean.mkv')], process.cwd());
